...
func TestEstablishShares_Success(t *testing.T) {
	...
	assert.Equal(t, "share1", share.GetName())
	assert.Equal(t, "yes", share.GetUseCache())
	require.NotNil(t, share.CachePool, "CachePool should not be nil")
	assert.Equal(t, "cache", share.GetCachePool().GetName())
	assert.Nil(t, share.GetCachePool2(), "CachePool2 should be nil")
	assert.Equal(t, "highwater", share.GetAllocator())
	assert.False(t, share.GetDisableCOW())
	assert.Equal(t, 0, share.GetSplitLevel())
	assert.Equal(t, uint64(10000000), share.GetSpaceFloor())
