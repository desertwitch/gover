...
// IOTargetQueue is a queue where items of a common target storage name
// were previously enqueued and aggregated by their [IOManager].
//
...
type IOTargetQueue struct {
	*GenericQueue[*schema.Moveable]
	...
	// bytesTransfered is the amount of bytes transferred for the [IOTargetQueue].
	bytesTransfered uint64
}

// NewIOTargetQueue returns a pointer to a new [IOTargetQueue].
// This method is generally only called from the respective [IOManager].
